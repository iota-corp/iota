...
def rule(event):
    ...
    if (
        deep_get(event, "protoPayload", "response", "status") == "Failure"
        or deep_get(event, "protoPayload", "methodName") != "io.k8s.core.v1.pods.create"
    ):
        return False
        ...
        if (
            ...
            for security_context in containers_info:
                ...
                if (
                    ...
                    or deep_get(security_context, "securityContext", "runAsNonRoot")
                    is False
                ):
                    return True
...
def alert_context(event):
    ...
    context["pod_security_context"] = [
        i.get("securityContext") for i in containers_info
    ]
    return context
