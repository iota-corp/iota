...
from kubernetes_helpers import get_ipinfo_asn, k8s_alert_context, k8s_udm
...
def is_cloud_infrastructure_ip(event, cloud_provider):
    """Check if source IP is from cloud provider infrastructure using enrichment ASN."""
...
def _is_legitimate_eks_node(event):
    ...
    username = k8s_udm(event, "username") or ""
...
def is_aws_managed_service(event):
    ...
    arn = deep_get(event, "user", "extra", "arn", default=[""])[0]
...
def _is_legitimate_aks_request(event, username):
    ...
    if username in AKS_MANAGED_SERVICE_ACCOUNTS:
        return True
    return username.startswith("system:node:") and is_cloud_infrastructure_ip(
        event, "azure"
    )
...
def rule(event):  # pylint: disable=too-many-return-statements
    ...
    response_status = k8s_udm(event, "responseStatus") or {}
    log_type = event.get("p_log_type", "")
...
def title(event):
    ...
    source_ips = k8s_udm(event, "sourceIPs") or ["<UNKNOWN_IP>"]
