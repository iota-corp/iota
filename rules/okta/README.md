...
| `okta_api_key_revoked` | API token revoked (Panther: Okta.APIKeyRevoked). | INFO |
| `okta_user_mfa_reset` | Detects MFA factor reset (single). | INFO |
| `okta_user_mfa_reset_all` | All MFA factors reset for a user (Panther: Okta.User.MFA.Reset.All). | MEDIUM |
| `okta_support_reset` | Password or MFA reset by Okta Support (Panther: Okta.Support.Reset). | HIGH |
...
| `okta_group_admin_role_assigned` | Admin privileges assigned to a group (Panther: Okta.Group.Admin.Role.Assigned). | INFO |
| `okta_rate_limits` | Rate limit / concurrency violation events (Panther: Okta.Rate.Limits). | MEDIUM |
| `okta_anonymizing_vpn_login` | Sign-in from anonymizing VPN/proxy (Panther: Okta.Anonymizing.VPN.Login). | MEDIUM |
| `okta_org2org_creation_modification` | Org2Org app created or modified (Panther: Okta.Org2org.Creation.Modification). | HIGH/MEDIUM |
| `okta_password_extraction_via_scim` | Cleartext passwords extracted via SCIM app (Panther: Okta.Password.Extraction.via.SCIM). | HIGH |
| `okta_app_unauthorized_access_attempt` | Unauthorized access attempt to an app (Panther: Okta.App.Unauthorized.Access.Attempt). | MEDIUM |
| `okta_app_refresh_access_token_reuse` | Refresh token reuse detected (Panther: Okta.Refresh.Access.Token.Reuse). | HIGH |
...
Additional rules ported from Panther are documented in [docs/ported-rules.md](../../docs/ported-rules.md).
