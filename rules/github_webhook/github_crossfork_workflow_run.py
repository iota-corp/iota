...
from github_helpers import is_cross_fork_pr


def rule(event):
    return (
        deep_get(event, "workflow_run", "event")
        in ("pull_request_target", "pull_request")
...
def title(event):
    workflow_name = deep_get(
        event, "workflow_run", "name", default="<UNKNOWN_WORKFLOW>"
    )
