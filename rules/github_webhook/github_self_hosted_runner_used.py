...
def rule(event):
    ...
    runner_name = deep_get(event, "workflow_job", "runner_name", default="")
...
def title(event):
    ...
    runner_name = deep_get(
        event, "workflow_job", "runner_name", default="Unknown Runner"
    )
...
def alert_context(event):
    ...
    return {
        ...
        "actor": deep_get(event, "sender", "login"),
    }


def severity(event):
    # Public or forkable repos with self-hosted runners have a medium risk
    repo_visibility = deep_get(event, "repository", "visibility")
    allow_forking = deep_get(event, "repository", "allow_forking", default=False)
    is_private = deep_get(event, "repository", "private", default=True)
