...
def rule(event):
    ...
    if deep_get(event, "resource", "type") != "http_load_balancer":
        return False

    # Check enforced policy match
    enforced_policy = deep_get(
        event, "jsonPayload", "enforcedSecurityPolicy", default={}
    )
    ...
    preview_policy = deep_get(event, "jsonPayload", "previewSecurityPolicy", default={})
...
def title(event):
    remote_ip = deep_get(event, "httpRequest", "remoteIp", default="<UNKNOWN_IP>")
...
def alert_context(event):
    ...
    status_details = deep_get(
        event, "jsonPayload", "statusDetails", default="<UNKNOWN_STATUS>"
    )
    ...
    context = {
        ...
        "project_id": deep_get(event, "resource", "labels", "project_id"),
        "backend_service": deep_get(
            event, "resource", "labels", "backend_service_name"
        ),
        "forwarding_rule": deep_get(
            event, "resource", "labels", "forwarding_rule_name"
        ),
    }
