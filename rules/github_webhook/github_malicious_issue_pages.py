...
def rule(event):
    ...
    is_issue_event = (
        ...
        and deep_get(event, "issue", "state") == "open"
    )
    ...
    if is_issue_event:
        fields_to_check = [
            deep_get(event, "issue", "title"),
            deep_get(event, "issue", "body"),
        ]
...
def title(event):
    ...
    if event.get("issue"):
        issue_number = deep_get(
            event, "issue", "number", default="<UNKNOWN_ISSUE_NUMBER>"
        )
        user = deep_get(event, "issue", "user", "login", default="<UNKNOWN_USER>")
...
def alert_context(event):
    ...
    if event.get("issue"):
        issue_fields = {
            "title": deep_get(event, "issue", "title"),
            "body": deep_get(event, "issue", "body"),
        }
...
def reference(event):
    ...
    issue_url = deep_get(event, "issue", "html_url")
    if issue_url:
        return issue_url
