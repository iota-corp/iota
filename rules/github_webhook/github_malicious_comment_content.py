...
def rule(event):
    ...
    if event.get("comment") and action in ["created", "edited"]:
        if contains_bash_injection_pattern(deep_get(event, "comment", "body")):
            return True

    # Handle pull_request_review events
    if event.get("review") and action in ["submitted", "edited"]:
        if contains_bash_injection_pattern(deep_get(event, "review", "body")):
            return True
    return False
...
def title(event):
    ...
    if event.get("comment"):
        comment_id = deep_get(event, "comment", "id", default="<UNKNOWN>")
    ...
    if event.get("review"):
        review_id = deep_get(event, "review", "id", default="<UNKNOWN>")
...
def alert_context(event):
    ...
    if comment_body := deep_get(event, "comment", "body"):
    ...
    # Analyze review body
    if review_body := deep_get(event, "review", "body"):
...
def reference(event):
    ...
    if comment_url := deep_get(event, "comment", "html_url"):
        return comment_url

    if review_url := deep_get(event, "review", "html_url"):
        return review_url
