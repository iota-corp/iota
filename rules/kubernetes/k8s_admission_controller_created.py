...
def title(event):
    ...
    resource = k8s_udm(event, "resource") or "webhook"
    name = k8s_udm(event, "name") or "<UNKNOWN_NAME>"
...
def severity(event):
    ...
    webhooks = k8s_udm(event, "webhooks") or []
...
def alert_context(event):
    ...
    return k8s_alert_context(
        event,
        extra_fields={
            "webhook_name": k8s_udm(event, "name"),
            "webhook_type": k8s_udm(event, "resource"),
